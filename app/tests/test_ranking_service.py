...
def mock_token() -> Token:
    return Token(
        chain=Chain.BASE, contract_address="0xcompute_all", name="All", symbol="ALL",
...
async def test_compute_all_with_tokens_returns_count(mock_repos):
    service = RankingService(
        mock_repos["token_repo"], mock_repos["snapshot_repo"],
        mock_repos["alpha_repo"], mock_repos["contract_security_repo"],
        mock_repos["social_repo"], mock_repos["dev_repo"],
    )
