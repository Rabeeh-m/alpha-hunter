...
class TestGoPlusClient:
    ...
    async def test_get_token_security_parses_response(self, http_client, mock_cache):
        respx.get(
            "https://api.gopluslabs.io/api/v1/token_security/8453?contract_addresses=0xabc"
        ).mock(return_value=httpx.Response(200, json=MOCK_SECURITY_RESPONSE))
    ...
    async def test_returns_none_when_address_not_in_result(self, http_client, mock_cache):
        respx.get(
            "https://api.gopluslabs.io/api/v1/token_security/8453?contract_addresses=0xmissing"
        ).mock(return_value=httpx.Response(200, json=MOCK_SECURITY_RESPONSE_NO_RESULT))
        client = GoPlusClient(http_client=http_client)
    ...
    async def test_uses_cached_value_when_available(self, http_client):
        ...
        monkeypatch.setattr(
            "app.collectors.goplus_client.cache_get",
            AsyncMock(
                return_value={
                    "is_open_source": "1",
                    "is_proxy": "0",
    ...
    async def test_retry_then_succeeds_after_transient_500(self, http_client, mock_cache):
        ...
        respx.get(
            "https://api.gopluslabs.io/api/v1/token_security/8453?contract_addresses=0xretry"
    ...
    async def test_raises_after_exhausting_retries(self, http_client, mock_cache):
        respx.get(
            "https://api.gopluslabs.io/api/v1/token_security/8453?contract_addresses=0xfail"
