...
async def list_token_wallets(
    token_id: UUID, db: AsyncSession = Depends(get_db)
) -> list[WalletHoldingRead]:
    ...
    for holding in holdings:
        ...
        results.append(
            WalletHoldingRead(
                ...
                approximate_balance=holding.approximate_balance,
                rank=holding.rank,
                scanned_at=holding.scanned_at,
            )
        )
    return results
...
async def list_recent_whale_events(
...
@router.get(
    "/whale-events", response_model=list[WhaleEventRead]
)  # note: mounted on the existing `router`, so path is /tokens/{token_id}/wallets/whale-events
async def list_token_whale_events(
    token_id: UUID, limit: int = 50, db: AsyncSession = Depends(get_db)
) -> list[WhaleEventRead]:
    repo = WhaleEventRepository(db)
