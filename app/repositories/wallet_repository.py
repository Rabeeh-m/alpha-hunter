...
class WalletRepository(BaseRepository[Wallet]):
    ...
    async def get_or_create(
        ...
        if existing is not None:
            ...
            if confidence is not None and (existing.confidence_score is None or confidence > existing.confidence_score):
                existing.wallet_type = wallet_type
                existing.confidence_score = confidence
            return existing

        wallet = Wallet(chain=chain, address=address, wallet_type=wallet_type, confidence_score=confidence)
