...
class SocialScoreRepository(BaseRepository[SocialScore]):
    ...
    async def get_by_token_id(self, token_id: UUID) -> SocialScore | None:
        result = await self.session.execute(select(SocialScore).where(SocialScore.token_id == token_id))
        return result.scalar_one_or_none()

    async def upsert(self, token_id: UUID, score: int, factor_breakdown: dict, possible_inorganic_growth: bool) -> SocialScore:
